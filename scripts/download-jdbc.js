			...
			const request = https.get(
				...
				response => {
					...
					if (response.statusCode === 301 || response.statusCode === 302) {
						...
						return this.downloadFile(redirectUrl, destination, driver).then(resolve).catch(reject);
					}
					...
					response.on('data', chunk => {
						...
						if (totalBytes > 0 && !this.verbose) {
							...
							process.stdout.write(
								`\r   Progress: ${percent}% (${this.formatBytes(downloadedBytes)}/${this.formatBytes(totalBytes)})`,
							);
						}
					});
