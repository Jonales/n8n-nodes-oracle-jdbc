  ...
  async checkJavaInstallation() {
    ...
    try {
      ...
      if (fs.existsSync(localJavaPath)) {
        const localJava = await this.execCommand(`${localJavaPath} -version 2>&1`, { silent: true });
    ...
    try {
      if (this.hasCommand('brew')) {
        ...
        if (fs.existsSync(brewJavaPath)) {
          await this.execCommand(`ln -sfn ${brewJavaPath}/libexec/openjdk.jdk /Library/Java/JavaVirtualMachines/openjdk-${this.javaVersionRequired}.jdk`);
        }
        return;
      }
