...
const PATHS = {
  ...
  // Watch patterns
  watch: {
    typescript: ['nodes/**/*.ts', 'credentials/**/*.ts', 'core/**/*.ts', 'types/**/*.ts', 'utils/**/*.ts'],
...
function copyIcons() {
  ...
  return gulp.src(PATHS.src.icons, { base: '.' })
...
async function copyLibraries() {
  ...
  try {
    ...
    const missingDrivers = CONFIG.oracle.requiredDrivers.filter(
      driver => !files.includes(driver)
    );
    ...
    const optionalMissing = CONFIG.oracle.optionalDrivers.filter(
      driver => !files.includes(driver)
    );
  ...
  return gulp.src(PATHS.src.lib, { base: '.' })
    .pipe(gulp.dest(PATHS.dist.base));
}
...
function copyDocs() {
  ...
  return gulp.src(PATHS.src.docs, { base: '.' })
    .pipe(gulp.dest(PATHS.dist.base));
}
...
function copyPackageFiles() {
  ...
  return gulp.src(PATHS.src.packageFiles, { base: '.' })
    .pipe(gulp.dest(PATHS.dist.base));
}
...
const buildComplete = gulp.series(
  cleanDist,
  copyAllAssets,
  compileTypeScript,
  validateJdbcDrivers
