...
export interface OracleTablespaceInfo {
  ...
  encrypted: 'YES' | 'NO';
  compressFor: 'BASIC' | 'OLTP' | 'QUERY LOW' | 'QUERY HIGH' | 'ARCHIVE LOW' | 'ARCHIVE HIGH' | null;
}
...
export namespace OracleTypeUtils {
  export function isNumericType(oracleType: string): boolean {
    const numericTypes = ['NUMBER', 'FLOAT', 'BINARY_FLOAT', 'BINARY_DOUBLE', 'INTEGER', 'SMALLINT'];
  ...
  export function isOracleReservedWord(word: string): boolean {
    const reservedWords = [
      ...
      'VIEW', 'SEQUENCE', 'TRIGGER', 'FUNCTION', 'PROCEDURE', 'PACKAGE', 'TYPE', 'CONSTRAINT',
      'PRIMARY', 'FOREIGN', 'UNIQUE', 'CHECK', 'NOT', 'NULL', 'DEFAULT', 'REFERENCES',
      'CASCADE', 'RESTRICT', 'ON', 'AND', 'OR', 'IN', 'LIKE', 'BETWEEN', 'EXISTS',
      'UNION', 'INTERSECT', 'MINUS', 'ORDER', 'GROUP', 'HAVING', 'WHERE', 'FROM',
      'JOIN', 'INNER', 'LEFT', 'RIGHT', 'FULL', 'OUTER', 'CROSS', 'NATURAL',
      'CONNECT', 'START', 'WITH', 'PRIOR', 'LEVEL', 'ROWNUM', 'SYSDATE', 'USER',
      'DUAL', 'ALL', 'ANY', 'SOME', 'AS', 'CASE', 'WHEN', 'THEN', 'ELSE', 'END',
      'IF', 'ELSIF', 'ELSE', 'LOOP', 'WHILE', 'FOR', 'EXIT', 'CONTINUE', 'GOTO',
      'RETURN', 'BEGIN', 'DECLARE', 'EXCEPTION', 'RAISE', 'PRAGMA', 'CURSOR',
      'FETCH', 'OPEN', 'CLOSE', 'COMMIT', 'ROLLBACK', 'SAVEPOINT', 'LOCK',
      'GRANT', 'REVOKE', 'PUBLIC', 'ROLE', 'USER', 'IDENTIFIED', 'BY', 'PASSWORD'
    ];
  ...
  export function formatOracleIdentifier(identifier: string): string {
    ...
    if (isOracleReservedWord(identifier) || 
  ...
  export function getDefaultPrecisionScale(oracleType: string): { precision?: number; scale?: number } {
    const type = oracleType.toUpperCase();
