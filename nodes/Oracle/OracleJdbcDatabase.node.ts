      ...
      for (let i = 0; i < items.length; i++) {
        ...
        switch (procedureType) {
          ...
          case 'package':
            ...
            result = await procExecutor.callPackageProcedure(packageName, packageProcedureName, parameters);
            break;
          default:
            throw new NodeOperationError(this.getNode(), `Unknown procedure type: ${procedureType}`);
        }
  ...
  private async executeWithPool(
    ...
    try {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        try {
          const result = await JdbcConnectionManager.getInstance()
            .executeQuery(connection, 'SELECT 1 as status FROM dual');
