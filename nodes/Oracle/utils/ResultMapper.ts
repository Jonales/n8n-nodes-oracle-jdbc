...
export class ResultMapper {
	...
	static mapOracleTypes(value: any, columnType: string, options: ResultMappingOptions = {}): any {
	...
	private static mapNumberType(value: any, options: ResultMappingOptions): number {
		if (typeof value === 'number') {
			return options.formatNumbers && options.decimalPlaces !== undefined
				? Number(value.toFixed(options.decimalPlaces))
				: value;
		}
		...
		return options.formatNumbers && options.decimalPlaces !== undefined
			? Number(numValue.toFixed(options.decimalPlaces))
			: numValue;
	}
	...
	private static mapGenericType(value: any, type: string, options: ResultMappingOptions): any {
	...
	private static handleNull(nullHandling: string, columnType: string): any {
		switch (nullHandling) {
			case 'empty':
	...
	private static normalizeColumnName(columnName: string, options: ResultMappingOptions): string {
	...
	static getCommonTransformations(): { [name: string]: ColumnTransformation } {
		return {
			...
			nullToEmpty: this.createTransformation('*', value => (value === null ? '' : value)),

			emptyToNull: this.createTransformation('*', value => (value === '' ? null : value)),
