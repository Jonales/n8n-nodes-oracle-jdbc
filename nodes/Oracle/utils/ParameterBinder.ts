...
import { ParameterDefinition, ParameterType, ParameterMode } from '../types/JdbcTypes';
...
export class ParameterBinder {
    ...
    for (let i = 0; i < parameters.length; i++) {
      ...
      if (param.constraints) {
        ...
        if (!constraintValidation.valid) {
          errors.push(...constraintValidation.errors.map(err => ({
            parameterIndex: i,
            parameterName: param.name,
            error: err,
            expectedType,
    ...
    switch (targetType) {
      case 'string':
        if (Array.isArray(value)) {
          return options.arrayHandling === 'join' ? value.join(',') :
                 options.arrayHandling === 'json' ? JSON.stringify(value) :
                 String(value);
        }
        if (typeof value === 'object') {
          return options.objectHandling === 'json' ? JSON.stringify(value) :
                 options.objectHandling === 'toString' ? String(value) :
                 '[object Object]';
        }
        let strValue = String(value);
        if (options.truncateStrings && options.maxStringLength && 
  ...
  private static validateConstraints(
    ...
    if (typeof value === 'string') {
      ...
      if (constraints.pattern) {
        const regex = constraints.pattern instanceof RegExp ? 
                     constraints.pattern : new RegExp(constraints.pattern);
        if (!regex.test(value)) {
  ...
  static createParameters(
    ...
    return values.map((value, index) => 
      this.createParameter(value, types?.[index])
    );
  }
