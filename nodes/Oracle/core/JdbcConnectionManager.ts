...
import { OracleJdbcConfig, JdbcConnection, QueryResult, QueryOptions } from '../types/JdbcTypes';
...
export class JdbcConnectionManager {
  ...
  async createConnection(config: OracleJdbcConfig, options: QueryOptions = {}): Promise<JdbcConnection> {
  ...
  async executeQuery(
    ...
    try {
      ...
      const isCall = sql.trim().toLowerCase().startsWith('call') || 
                     sql.trim().startsWith('{');
      ...
      if (isSelect) {
        const resultSet = parameters && parameters.length > 0
        ...
        const updateCount = parameters && parameters.length > 0
