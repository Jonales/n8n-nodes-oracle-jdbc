...
export class OracleJdbcDriver {
  ...
  static async initialize(options: DriverInitializationOptions = {}): Promise<void> {
    ...
    try {
      ...
      throw ErrorHandler.handleJdbcError(error, 'Failed to initialize Oracle JDBC driver', errorContext);
    }
  }
  ...
  static async testConnection(config: OracleJdbcConfig, options: {
    timeout?: number;
  ...
  static async validateConfiguration(config: OracleJdbcConfig): Promise<{
      ...
      case 'tns':
        if (!config.tnsString || config.tnsString.trim() === '') {
          ...
          if (!config.tnsString.includes('DESCRIPTION') || !config.tnsString.includes('ADDRESS')) {
            warnings.push('TNS string may be malformed - should contain DESCRIPTION and ADDRESS clauses');
          }
        }
        break;
    ...
    if (config.connectionOptions) {
      if (config.connectionOptions.connectionTimeout && config.connectionOptions.connectionTimeout <= 0) {
  ...
  static async createOptimizedConnection(config: OracleJdbcConfig, options: {
    autoCommit?: boolean;
    readOnly?: boolean;
    schema?: string;
  ...
  private static async configureGlobalProperties(): Promise<void> {
    try {
      ...
      await System.setProperty('oracle.jdbc.fanEnabled', 
        this.initializationOptions.enableFan ? 'true' : 'false');

      await System.setProperty('oracle.net.keepAlive', 
        this.initializationOptions.keepAlive ? 'true' : 'false');

      if (this.initializationOptions.enableOracleOptimizations) {
        // Performance optimizations
        await System.setProperty('oracle.jdbc.defaultRowPrefetch', 
          this.initializationOptions.defaultRowPrefetch!.toString());

        await System.setProperty('oracle.jdbc.defaultBatchValue', 
          this.initializationOptions.defaultBatchValue!.toString());

        await System.setProperty('oracle.jdbc.implicitStatementCacheSize', 
  ...
  private static buildUrlParameters(config: OracleJdbcConfig, sslConfig?: SSLConfiguration): string[] {
    const params: string[] = [];
    ...
    if (config.connectionOptions?.connectionTimeout) {
      params.push(`oracle.net.CONNECT_TIMEOUT=${config.connectionOptions.connectionTimeout * 1000}`);
    }
