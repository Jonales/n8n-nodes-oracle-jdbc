...
import { JdbcConnection, QueryResult, QueryOptions, ColumnMetadata } from '../types/JdbcTypes';
...
export class QueryExecutor {
  ...
  async executeQuery(
    ...
    } else if (trimmedSql.startsWith('begin') || trimmedSql.startsWith('declare') || 
               trimmedSql.includes('{call') || trimmedSql.includes('{ call')) {
  ...
  private async extractResultSet(resultSet: any, options: ExecutionOptions = {}): Promise<any[]> {
    ...
    while (await resultSet.next() && rowCount < maxRows) {
      ...
      if (options.streamResults && options.chunkSize && rowCount > 0 && rowCount % options.chunkSize === 0) {
  ...
  private groupQueriesBySQL(queries: Array<{ sql: string; parameters?: any[] }>): Map<string, Array<{ originalIndex: number; query: { sql: string; parameters?: any[] } }>> {
  ...
  private async executeSQLBatch(sql: string, queryData: any[], options: ExecutionOptions): Promise<QueryResult[]> {
  ...
  private generatePlanHash(planRows: any[]): string {
    const planString = planRows.map(row => 
      `${row.OPERATION || ''}-${row.OPTIONS || ''}-${row.OBJECT_NAME || ''}`
    ).join('|');
