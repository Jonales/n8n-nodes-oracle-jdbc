...
export class ConnectionPool {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw ErrorHandler.handleJdbcError(error, 'Failed to initialize connection pool', errorContext);
    }
  }
  ...
  async returnConnection(connection: JdbcConnection): Promise<void> {
    ...
    try {
      ...
      throw ErrorHandler.handleJdbcError(error, 'Failed to return connection to pool', errorContext);
    }
  }
  ...
  private async configurePoolProperties(): Promise<void> {
    ...
    if (this.poolConfig.abandonedConnectionTimeout) {
      await this.dataSource.setAbandonedConnectionTimeout(this.poolConfig.abandonedConnectionTimeout);
    }

    if (this.poolConfig.removeAbandoned !== undefined) {
      await this.dataSource.setConnectionProperty('oracle.ucp.removeAbandoned', this.poolConfig.removeAbandoned.toString());
    }
  }
  ...
  private async configureOracleProperties(): Promise<void> {
    ...
    await this.dataSource.setConnectionProperty('oracle.jdbc.defaultRowPrefetch', 
      (this.poolConfig.defaultRowPrefetch || 20).toString());
    await this.dataSource.setConnectionProperty('oracle.jdbc.defaultBatchValue',
  ...
  private async configureValidationProperties(): Promise<void> {
    ...
    if (this.poolConfig.validateConnectionOnReturn) {
      await this.dataSource.setValidateConnectionOnReturn(this.poolConfig.validateConnectionOnReturn);
    }
    ...
    if (this.poolConfig.connectionValidationQuery) {
      await this.dataSource.setConnectionProperty('oracle.ucp.connectionValidationQuery', 
        this.poolConfig.connectionValidationQuery);
    }

    if (this.poolConfig.connectionValidationTimeout) {
      await this.dataSource.setConnectionProperty('oracle.ucp.connectionValidationTimeout',
  ...
  private startMonitoring(): void {
    ...
    this.monitoringTimer = setInterval(async () => {
      try {
        ...
        if (stats.connectionLeaks > 0) {
          console.warn(`Pool ${stats.poolName} has ${stats.connectionLeaks} potential connection leaks`);
        }
    ...
    for (const [connectionId, borrowTime] of this.activeConnections) {
      if (now - borrowTime.getTime() > leakThreshold) {
        ...
        if (this.poolConfig.logAbandoned) {
          console.warn(`Potential connection leak detected: ${connectionId}, borrowed ${Math.floor((now - borrowTime.getTime()) / 1000)}s ago`);
        }
      }
    }
  ...
  private updateConnectionStatistics(operation: 'borrowed' | 'returned' | 'failed', waitTime?: number): void {
    switch (operation) {
      case 'borrowed':
        ...
        if (waitTime) {
          this.statistics.maxConnectionWaitTime = Math.max(this.statistics.maxConnectionWaitTime, waitTime);
          // Update average (simple moving average)
          this.statistics.averageConnectionWaitTime = 
            (this.statistics.averageConnectionWaitTime * (this.statistics.connectionRequestsCount - 1) + waitTime) / 
            this.statistics.connectionRequestsCount;
        }
        break;
      case 'failed':
