...
export interface TransactionOperation {
  timestamp: Date;
  operation: 'BEGIN' | 'COMMIT' | 'ROLLBACK' | 'SAVEPOINT' | 'ROLLBACK_TO_SAVEPOINT' | 'RELEASE_SAVEPOINT';
  description?: string;
...
export class TransactionManager {
  ...
  async createSavepoint(name?: string, description?: string): Promise<string> {
    ...
    try {
      ...
      throw ErrorHandler.handleJdbcError(error, `Failed to create savepoint: ${savepointName}`, errorContext);
    }
  }
  ...
  async releaseSavepoint(name: string): Promise<void> {
    ...
    try {
      ...
      throw ErrorHandler.handleJdbcError(error, `Failed to release savepoint: ${name}`, errorContext);
    }
  }
  ...
  async executeBatch(operations: Array<() => Promise<any>>, options: {
