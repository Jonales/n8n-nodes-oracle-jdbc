...
export class StoredProcedureExecutor {
  ...
  async callProcedure(
    ...
    try {
      ...
      const { resultSets, cursors } = await this.extractAllResultSets(callableStatement, parameters, options);
  ...
  async callFunction(
    ...
    try {
      ...
      const callSql = parameters.length > 0
  ...
  private async extractAllResultSets(
    ...
    for (const param of parameters) {
      if (param.dataType.toUpperCase() === 'CURSOR' && (param.type === 'OUT' || param.type === 'INOUT')) {
        try {
  ...
  static createInOutParameter(name: string, dataType: string, value: any, size?: number): ProcedureParameter {
